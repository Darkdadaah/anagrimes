...
use wiktio::pron_tools
  qw(cherche_prononciation simple_prononciation section_prononciation);
our %opt;
...
sub article {
    ...
    # Pages
    if ( defined( $opt{q} )
        and not $article->[0] =~ /pagequality level=&quot;[($opt{q})]&quot;/ )
    {
        return 0;
    }
    ...
    # Normal articles
    if ( defined( $opt{Q} )
        and not $article->[0] =~ /\{\{TextQuality\|($opt{Q})\}\}/ )
    {
        return 0;
    }
...
# Pas de nombre ni de signe de ponctuation ni de symbole ou d'autre truc bizarre
        ...
        next
          if $mot =~
/-(je|tu|il|elle|on|nous|vous|ils|elles|moi|toi|lui|leur|là|ci|ce|le|la|les|y|m|t|en)?$/;
   ...
   # Si avec historique : vérifier s'il y a une version plus récente (=après)
    ...
    if ($complete_article) {
        ...
        if ( $article[0] =~ /#redirect/i ) {
            ...
            ######################################
            $count{pages}++;
            print STDERR "[$count{pages}] [$count{mots}] $title          \r"
              if $count{pages} % 1000 == 0;
        }
        $complete_article = 0;
    }
}
