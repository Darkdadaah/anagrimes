...
use wiktio::pron_tools
  qw( cherche_prononciation cherche_transcription simple_prononciation extrait_rimes section_prononciation nombre_de_syllabes );
...
sub init() {
    ...
    foreach my $type ( keys %output_files ) {
        ...
        if ( $opt{i} ) {
            open( TYPE, "> $output_files{$type}{file}" )
              or die "Can't init file $output_files{$type}{file}: $!\n";
            close(TYPE);
        }
    }

    if ( $opt{c} ) {
        usage("Wrong number for the crossword columns (-c): $opt{c}")
          if not "$opt{c}" =~ /^[1-9][0-9]*$/;
        $max_col = $opt{c};
...
sub print_sql_schema {
    ...
    foreach my $table ( sort keys %output_files ) {
        print_sql_table(
            $SQL, $table,
            $output_files{$table}{fields},
            $output_files{$table}{constraints}
        );
    }
...
"CREATE VIEW entries AS SELECT * FROM articles INNER JOIN lexemes ON a_artid=l_artid LEFT JOIN prons ON l_lexid=p_lexid;\n";
    print $SQL
"CREATE VIEW defentries AS SELECT * FROM articles INNER JOIN lexemes ON a_artid=l_artid LEFT JOIN prons ON l_lexid=p_lexid INNER JOIN defs ON l_lexid=d_lexid;\n";
    print $SQL
"CREATE VIEW deflex AS SELECT * FROM articles INNER JOIN lexemes ON a_artid=l_artid INNER JOIN defs ON l_lexid=d_lexid;\n";
    ...
    foreach my $table (
        sort { $output_files{$a}{'order'} <=> $output_files{$b}{'order'} }
        keys %output_files )
    {
        ...
        if ( $dbtype eq 'sqlite' ) {
            ...
            print $SQL
"LOAD DATA LOCAL INFILE '$file' INTO TABLE $table CHARACTER SET 'utf8';\n";
        }
        else {
...
sub add_to_file {
    ...
    open( TYPE, ">> $output_files{$type}{file}" )
      or die "Can't write $output_files{$type}{file} (type $type): $!\n";
...
sub add_to_file_lang {
    my ($line) = @_;
    open( LANG, "> $output_files{langs}{file}" )
      or die "Can't write $output_files{langs}{file}: $!\n";
    foreach my $l ( sort keys(%lang_total) ) {
        my $l_rand =
          $lang_filter{$l}{'l_rand'} ? $lang_filter{$l}{'l_rand'} : 0;
        my %values = (
            'lg_lang'     => $l,
            'lg_num'      => $lang_total{$l},
            'lg_rand_num' => $l_rand,
...
sub title_format {
    ...
    $title{'a_title_r'} =
      reverse( $title{'a_title'} );    # Reverse title (for sql search)
    $title{'a_title_flat'} =
      lc( ascii_strict( $title{'a_title'} ) );    # titre_plat (no hyphenation)
    $title{'a_title_flat_r'} =
      reverse( $title{'a_title_flat'} );          # Same, reversed
    $title{'a_alphagram'} = anagramme( $title{'a_title_flat'} )
      ;    # anagramme_id (alphagram, key for anagrams)
...
sub transc_format {
    ...
    if ( not unicode_NFKD( $title->{'a_title_flat'} ) =~ /[a-z]/ ) {
        ...
        $transc{'a_trans_flat'} =
          transcription( $title->{'a_title_flat'}, \@langs );
        ...
        {
            special_log( 'incomplete_transcription', $title->{'a_title'},
                $transc{'a_trans_flat'} );
            $transc{'a_trans_flat'} = '';
...
sub parse_article {
    ...
    if ( not $title_val->{'a_title_flat'} ) {
        special_log( 'a_title_flat', $title_val->{'a_title'} )
          ;    # Log just to be sure
        ...
        my $article_section =
          parseArticle( $article->{'content'}, $article->{'title'}, $wlang );
        ...
        if ( $opt{L} ) {
            ...
            if ( $#{$lang_section} + 1 > 0 ) {
                parse_language_sections( $article, $lang_section, $wlang,
                    $lang );
                $lang_ok = $true;
            ...
            foreach my $lang ( sort keys %{ $article_section->{language} } ) {
                ...
                if ( not $lang_section ) {
                    special_log( 'empty_lang', $title_val->{'a_title'}, $lang )
                      ;    # Log just to be sure
        ...
        my $transc_val =
          transc_format( $title_val, $article, $article_section );
...
sub parse_language_sections {
    ...
    my @prononciations =
      section_prononciation( $lang_section->{'prononciation'}->{lines},
        $title );
    my %transc =
      ();    # Prepare to store every transcription found in the text as well
...
#  Special, name of inhabitants should be marked to be avoidable in searches as there is *a lot*
        my $gent =
          is_gentile( $lang_section->{'type'}->{$type}->{lines}, $type_nom );

        #  Special, is it an initialism or an abreviation?
        my $sigle = is_sigle( $lang_section->{'type'}->{$type}->{lines},
            $lang, $title, $type_nom );

        # Fing genera
        my $genre = cherche_genre( $lang_section->{'type'}->{$type}->{lines},
            $lang, $title, $type_nom );

# Get all pronunciations for this word that we can find in the text (marked with models usually)
        my $prons =
          cherche_prononciation( $lang_section->{'type'}->{$type}->{lines},
            $lang, $title, $type, $flex );
