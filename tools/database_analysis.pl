...
sub get_articles {
    ...
    my $query =
"SELECT DISTINCT(a_title) FROM articles JOIN lexemes ON a_artid=l_artid WHERE $conditions";
...
sub pronunciations {
    ...
    my ( $total, $same, $samer, $sameish ) =
      expected_pronunciation( $articles, $lang );
    my $nsame    = $total - @$same;
    my $nsamer   = $total - @$samer;
    my $nsameish = $total - @$sameish;
    print STDERR "$total\tarticles with usable pronunciations\n";
    print STDERR "$nsame\tdifferent "
      . sprintf( "(%.2f %%)\n", $nsame / $total * 100 );
    print STDERR "$nsamer\tquite different "
      . sprintf( "(%.2f %%)\n", $nsamer / $total * 100 );
    print STDERR "$nsameish\tcompletely different "
      . sprintf( "(%.2f %%)\n", $nsameish / $total * 100 );
}
...
sub expected_pronunciation {
    ...
    foreach my $a (@$articles) {
        my ( $correct, $is_same, $is_samer, $is_sameish ) =
          check_pronunciation( $a, $lang );
        $count += $correct;
...
sub check_pronunciation {
    ...
    if ( $lang eq 'fr' ) {
        ...
        if ($pron) {
            ...
            print STDOUT "$diffs\t$art->{'a_title'}\t'$pron'\t'"
              . clean_pron( $art->{p_pron} ) . "'\n";
...
sub pron_in_fr {
    ...
    $voy .= "|"
      . decode( 'utf8',
        "é|è|ê|ɛ|ɛ̃|É|È|œ|œ̃|i|Œ|ɑ|ɑ̃|ə|ɔ|ɔ̃|ø" );
...
sub print_letters {
    ...
    foreach my $char (
        sort { $letters->{$b}->{"letters"} <=> $letters->{$a}->{"letters"} }
        keys %$letters )
    {
        my @line = (
            ...
            $letters->{$char}->{'words'},
            sprintf( "%.3f",
                $letters->{$char}->{'letters'} / $stats->{'letters'} * 100 ),
            sprintf( "%.3f",
                $letters->{$char}->{'words'} / $stats->{'words'} * 100 ),
        );
