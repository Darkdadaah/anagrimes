...
package wiktio::transcription;

use Exporter;
@ISA    = ('Exporter');
@EXPORT = qw(transcription);
...
use wiktio::basic;
use wiktio::string_tools qw(unicode_NFKD);
...
sub transcription {
    ...
    foreach my $l (@$langues) {
        if ( $langues_transcrites->{'cyrillique'}->{$l} ) {
            ...
            $transcrit =~
tr/абвгґдежзіиклмнопрстфыэ/abvggdejziiklmnoprstfye/;
            $transcrit =~
tr/АБГҐДЕЖЗІИКЛМНОПРСТФЫЭ/ABGGDEJZIIKLMNOPRSTFYE/;
...
#print STDERR "[[$titre]]\tTranscription du cyrillique ratée : '$transcrit'\n";
        ...
        if ( $langues_transcrites->{'grec'}->{$l} ) {
            $transcrit =~
tr/αβϐγδεϵϝζηικϰλμνξοπϖρϱσςϲτυφχψω/abbgdêêwzeikklmnxopprrssstuô/;
            $transcrit =~
tr/ΑΒΓΔΕϜΖΗΙΚΛΜΝΞΟΠΡΣϹΤΥΦΧΨΩ/ABGDÊWZEIKLMNXOPRSSTUÔ/;
...
#print STDERR "[[$titre]]\tTranscription de l'arabe ratée : '$transcrit' (reste $len lettres : '$left')\n";
                special_log( 'bad_arabe', $titre,
                    "reste $len lettres : '$left'", $transcrit );
            }
            else {
     #print "[[$titre]]\tTranscription de l'arabe réussie : '$transcrit' !!\n";
        ...
        if ( $langues_transcrites->{'ja'}->{$l} ) {
            ...
            if ( not unicode_NFKD($transcrit) =~ /^[a-zâ'0-9 ]+$/ ) {
                ...
                $left =~ s/[a-zâ'0-9 ]//g;
                my $len = length($left);
                special_log( 'bad_ja', $titre, "reste $len lettres : '$left'",
                    $transcrit );
            }
            else {
